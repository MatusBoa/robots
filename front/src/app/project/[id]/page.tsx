...
export default async function Page({
  ...
  const [project, bots] = await Promise.all([getProject(id), getBots(id)])

  return (
    <div className='container mx-auto space-y-4'>
      <h1 className='font-semibold text-2xl'>{project.getName()}</h1>
          ...
          <TableBody>
            {bots.map((bot) => (
              <TableRow key={bot.getId()}>
                <TableCell>{bot.getName()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
